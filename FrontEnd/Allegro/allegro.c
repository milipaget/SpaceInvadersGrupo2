...
//Carga los bitmaps necesarios. Si est� todo bien devuelve OK_GRAPHICS. En caso contrario devuelve ERROR_GRAPHICS.
...
//Carga los samples para el menu. Si est� todo bien devuelve OK_GRSPHICS. En caso contrario devuelve ERROR_GRAPHICS.
...
int loadMenuSamples(ALLEGRO_SAMPLE* menuSamples[]) {
	...
	//Si todo est� bien devuelvo OK. En caso contrario devuelvo ERROR.
	if (err == 0) {
		return OK_GRAPHICS;
	}
	else {
...
int stateMenu(graphics_t* graphics) {
	...
	if (al_get_next_event(graphics->eventQueue, &ev)) {
		if(ev.type == ALLEGRO_EVENT_DISPLAY_CLOSE){
		   ...
		   // redraw = true;//Podr�amos probar para un redraw
		}
...
void printSpaceInvaders(graphics_t* graphics, player_t* player, alien_t* aliens, barriers_t* barriers, spaceship_t* spaceship, alienRandom_t* rAlien, int level, int frames){
	...
	//Imprime a los aliens en su posici�n y las balas que tengan.-----Ac� falta la parte del alien muerto(explota).
	...
	//Imprime al alien random.-----Ac� falta la parte del alien random muerto(explota).
	if(rAlien->alive==ALIVE){
			al_draw_bitmap(graphics->randomAlienBitmap, (rAlien->pos.x)*UNIDAD, (rAlien->pos.y)*UNIDAD, 0);
	}


	//Imprime las barreras.-----Ac� falta la parte de la barrera muerta(explota).
	...
	al_draw_textf(graphics->font, al_map_rgb(255, 255, 255), 0, FUENTE_SIZE/2, 0, "Puntaje %ld", player->points);
	updateGraphics();
...
void read_keyboard(player_t* player, graphics_t* graphics){
	...
	while (!exit_keyboard)
	{
		...
		//  1� TOMA EL �LTIMO EVENTO DE LA COLA DE EVENTOS
